...
5. **Module file:** `[name].module.ts` — explicit `imports`, `providers`, `exports`. Export only the interface/token, never internal implementation classes.
6. **Register in `app.module.ts`.**
7. **Update ESLint boundaries:** If this module should be isolated from another (like `analysis` from `google-places`), add a zone to `import/no-restricted-paths` in `eslint.config.mjs`. Ask the user before doing this if the boundary rule isn't obvious from existing patterns.
8. **Update `.agents/state.md`** once the module compiles and its checklist item is done.
