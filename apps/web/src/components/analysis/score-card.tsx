...
function ScoreRing({ score }: { score: number }) {
  ...
  if (score <= 42) color = '#ef4444'; // red-500
          ...
          <HoverCard openDelay={200} closeDelay={200}>
            ...
            <HoverCardContent side="top" align="center" className="w-96 p-5 shadow-xl border-gray-200">
              <div className="space-y-4">
                {[
                  { range: '0–24', color: 'bg-red-500', label: 'Critical', desc: 'Major optimization issues detected.' },
                  { range: '25–49', color: 'bg-red-500', label: 'Poor', desc: 'Basic optimization exists, but many important elements are missing.' },
                  { range: '50–64', color: 'bg-amber-500', label: 'Fair', desc: 'Profile is functional but requires several improvements.' },
                  { range: '65–79', color: 'bg-amber-500', label: 'Good', desc: 'Well optimized with noticeable room for improvement.' },
                  { range: '80–89', color: 'bg-green-500', label: 'Very Good', desc: 'Strong profile with only minor recommendations.' },
                  { range: '90–100', color: 'bg-green-500', label: 'Excellent', desc: 'Profile is highly optimized and follows best practices.' }
                ].map(band => (
                  <div key={band.label} className="flex items-start gap-3">
                    <div className="w-16 text-right font-medium text-gray-900 shrink-0 text-sm pt-0.5 tabular-nums">{band.range}</div>
